...
NanoDB is a versatile Python-based application that provides a graphical user interface (GUI) for entering, managing, and exporting laboratory experiment data. The application allows users to submit and edit data to a local offline database, generate PDF reports, and handle image files efficiently. This application is build based on FAIR principles for scientific data management.
